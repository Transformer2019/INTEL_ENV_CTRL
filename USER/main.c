...
//		}
		...
		warn_flag = warn_flag1 || warn_flag2 || warn_485temp_flag || warn_485rh_flag || warn_NH3;
		//printf("warn_flag:%d\n",warn_flag);
...
//									}	
								...
								case 3:
									...
									if(json_is_object(temp_time_ctrl)){
										json_t *max_temp_j = json_object_get(temp_time_ctrl, "max_temp");
										json_t *min_temp_j = json_object_get(temp_time_ctrl, "min_temp");
										json_t *temp_choose_flag_j = json_object_get(temp_time_ctrl, "temp_choose_flag");
										json_t *startup_mode_j = json_object_get(temp_time_ctrl, "startup_mode");
										json_t *time_stop_j = json_object_get(temp_time_ctrl, "time_stop");
										json_t *time_open_j = json_object_get(temp_time_ctrl, "time_open");
					...
					if(strstr(UART3_RxBuff,"+CCLK:")){
						...
						if (clock_str != NULL) {
							...
							if (strstr(time_str, "+32") != NULL) {
								...
								uint8_t second_t = 0;
								uint8_t tz_offset = 0;
								int result = sscanf(time_str, "%2d/%2d/%2d,%2d:%2d:%2d+%d",&year_t, &month_t, &day_t, &hour_t, &minute_t, &second_t, &tz_offset);
								if(result==7){
									year_t+=2000;
									if((hour_t+8)>=24){
										hour_t-=16;
										if ((year_t % 4 == 0 && year_t % 100 != 0) || (year_t % 400 == 0)) {
											// 闰年 366
											if (((month_t==1 || month_t==3 || month_t==5 || month_t==7 || month_t==8 || month_t==10) && day_t==31) || ((month_t==4 || month_t==6 || month_t==8 || month_t==11) && day_t==30) || (month_t==2 && day_t==29)) {
												day_t=1;month_t+=1;
											}else if(month_t==12 && day_t==31){
												year_t+=1;month_t=1;day_t=1;
											}else{
												day_t+=1;
											}
										} else {
											// 平年 365
											if (((month_t==1 || month_t==3 || month_t==5 || month_t==7 || month_t==8 || month_t==10) && day_t==31) || ((month_t==4 || month_t==6 || month_t==8 || month_t==11) && day_t==30) || (month_t==2 && day_t==28)) {
												day_t=1;month_t+=1;
											}else if(month_t==12 && day_t==31){
												year_t+=1;month_t=1;day_t=1;
											}else{
										...
										//赋值给系统时间
										RTC_Set(year_t,month_t,day_t,hour_t+8,minute_t,second_t);
									}
...
//			}
		...
		//if(TIM4_flag){
			...
			temperature1,temperature2,average_temp,warn_flag,out_voltage,send_NH3,send_RH,send_TEMP);
...
//			}
		...
		if(send_warn_Flag && mqtt_flag && network_flag){
			...
			sprintf(send_warn_config,"%d,%d,%d,%d,%d;%d,%d;%d,%d;%d,%d;%d,%d;%d",warn_temp1_flag,warn_temp2_flag,warn_temp485_flag,warn_rh_flag,NH3_warn_flag,limit_temp1_maxvalue,limit_temp1_minvalue,limit_temp2_maxvalue,limit_temp2_minvalue,limit_temp485_maxvalue,limit_temp485_minvalue,limit_rh_maxvalue,limit_rh_minvalue,NH3_max);//sprintf
...
//			u8 Heartbeat_len = strlen(send_data_Heartbeat);
		...
		if(network_flag==1 && MQTT_CON_flag){
			...
			MQTT_CON_Counter=0;
			MQTT_CON_flag=0;
			//LCD_ShowString(0,40,16,(char *)UART3_RxBuff,0);
			if(UART3_RxCounter != 0){
				...
				if(strstr(UART3_RxBuff,"+MQTTSTATE: 2")!=NULL){
					...
					UART3_Puts("AT+MQTTSUB=0,\"YKWL/CTRL/%s\",2\r\n",imei_no);
					delay_ms(10);
					//LCD_ShowString(0,40,16,"1564huiuhuii",0);
					//UART3_Puts("AT+MQTTSUB=0,\"testTopic/1234586\",1\r\n");
					UART3_RxCounter = 0; //重新等待接收下一个推送消息
					memset(UART3_RxBuff, 0, UART3_RXBUFF_SIZE); //将串口3接收缓冲区清0	
			...
			if(UART3_RxCounter != 0){
				...
				if(strstr(UART3_RxBuff,"+CCLK:")){
					...
					if (clock_str != NULL) {
						...
						if (strstr(time_str_test, "+32") != NULL) {
							...
							uint8_t second_t = 0;
							uint8_t tz_offset = 0;
							int result = sscanf(time_str_test, "%2d/%2d/%2d,%2d:%2d:%2d+%d",&year_t, &month_t, &day_t, &hour_t, &minute_t, &second_t, &tz_offset);
							if(result==7){
								year_t+=2000;
