...
//void LCD_ShowString_Color(u16 x,u16 y,u8 size,u8 *p,u8 mode)
//{         
//    while((*p<='~')&&(*p>=' '))//判断是不是非法字符!
//    {   
//		if(x>(lcddev.width-1)||y>(lcddev.height-1)) 
//		return;     
//		LCD_ShowChar(x,y,BACK_COLOR,POINT_COLOR,*p,size,mode);
//        x+=size/2;
//        p++;
//    }  
//} 
...
//	LCD_SetWindows(x,y,x+160-1,y+160-1);//窗口设置
//    for(i=0;i<160*160;i++)
//	{	
//	 	picL=*(p+i*2);	//数据低位在前
//		picH=*(p+i*2+1);				
...
//	}	
	...
	LCD_SetWindows(x,y,x+32-1,y+32-1);//窗口设置
    for(i=0;i<32*32;i++)
	{	
	 	picL=*(p+i*2);	//数据低位在前
		picH=*(p+i*2+1);				
...
{  
	...
	LCD_SetWindows(x,y,x+31,y+31); //设置区域      
	temp+=index*128;
...
void showhanzi_color(unsigned int x,unsigned int y,unsigned char index, u16 POINT_COLOR, u16 BACK_COLOR)	
{  
	unsigned char i,j;
	const unsigned char *temp=hanzi;    
...
void show_4g(unsigned int x,unsigned int y,unsigned char index)	
{  
	unsigned char i,j;
	unsigned char *temp=g4;    
  
	LCD_SetWindows(x,y,x+15,y+31); //设置区域      
	temp+=index*64;
...
void showdigit_color(unsigned int x,unsigned int y,unsigned char index, u16 POINT_COLOR, u16 BACK_COLOR)	
{  
	unsigned char i,j;
	unsigned char *temp=asc_32016;    
