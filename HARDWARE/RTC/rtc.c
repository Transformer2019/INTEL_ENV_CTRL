...
u8 RTC_Init(void)
{
    ...
    if (BKP_ReadBackupRegister(BKP_DR1) != 0x5051)		//从指定的后备寄存器中读出数据:读出了与写入的指定数据不相乎
    {
        ...
        RTC_WaitForLastTask();	//等待最近一次对RTC寄存器的写操作完成
        RTC_Set(2024,8,23,21,13,00);  //设置时间
...
//uint8_t  relay_No_tt[] = {0};
//Relay_Time_Seg *relay_time_seg[] = {NULL};
...
void RTC_IRQHandler(void)
{
    if (RTC_GetITStatus(RTC_IT_SEC) != RESET)//秒钟中断
    {
		...
		uint8_t  relay_No_tt[11] = {0};
		//谨慎使用指针，引起按键操作第三个风机模式为分时控时程序卡住
		//Relay_Time_Seg *relay_time_seg[] = {NULL};
		//问题所在：：声明数组指针时没有确定数组元素个数
		//取代方法1
		//Relay_Time_Seg *relay_time_seg[11] = {NULL};
		//取代方法2
		Relay_Time_Seg relay_time_segm[11] = {0};
	    ...
	    uint8_t t = 0;
		for(int i=0; i<10; i++){
			if(relay_structure[i].relay_mode == 4){
				//分时控参数
